import {
    Critter, TableData, isBug, isFish, isSealife,
  ...
  const sortOrderSpeed = ['none', 'extra slow', 'slow', 'medium', 'fast', 'extra fast'];
  ...
  const sortLocation = (order: 'ASC' | 'DSC') => (a: Critter, b: Critter) => {
    if (!('location' in a) || !('location' in b) || isSealife(a) || isSealife(b)) {
  ...
  const sortBubbles = (order: 'ASC' | 'DSC') => {
    ...
    return (a: Critter, b: Critter) => {
      if (!('bubble' in a) || !('bubble' in b) || !isSealife(a) || !isSealife(b)) {
  ...
  export type SortingType = 'value' | 'name' | 'location' | 'shadow' | 'bubbles' | 'speed' | 'notes' | 'pic';
  
  export const sortCritter = (type: SortingType, order: 'ASC' | 'DSC'): CritterSorting => {
    switch (type) {
      case 'value':
