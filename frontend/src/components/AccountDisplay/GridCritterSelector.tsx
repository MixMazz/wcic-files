import React, {
  ...
  const GridCritterSelector: React.FC<SelectorFieldsetProps> = ({
    ...
    const resetSelections = () => {
      ...
      logEvent('ResetGrid', {
        critterType: type,
        registeredUser: loggedInUser?._id,
      });
    };
    ...
    const fillerIcon = getFillerIconSVG(type);
    ...
    const submitNewCritterString = useCallback(async () => {
      ...
      try {
        const updatedUser = await changeCritterData(type, { encodedString });
    ...
    }, [changeCritterData, checkedboxes, loggedInUser]);
