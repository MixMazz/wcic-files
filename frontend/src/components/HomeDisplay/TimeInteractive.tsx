...
const TimeInteractive = () => {
  const { hour, month, meridiem, changeHour, changeMonth, toggleMeridiem, allDayOverride, setToRealTime, toggleTimeOverride } = useTimeContext();
  const [showRightNowBtn, setShowRightNowBtn] = useState(false);
      ...
      <div id="timeContainer">
        <div id="timeAlignment">
          <div id="timeInteractives">
            <div className={`timeKeeper ${allDayOverride ? 'timeOverride' : ''}`} onClick={() => setShowRightNowBtn(true)}>
              <TimeButton
                text={month}
                type="month"
